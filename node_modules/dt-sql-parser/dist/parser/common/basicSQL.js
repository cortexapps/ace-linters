...
export class BasicSQL {
     ...
     */
    getMinimumParserInfo(input, tokenIndex, originParseTree) {
        var _a, _b, _c, _d, _e, _f, _g, _h, _j;
        if (arguments.length <= 2) {
            this.parseWithCache(input);
            originParseTree = this._parseTree;
        }
        if (!originParseTree || !(input === null || input === void 0 ? void 0 : input.length))
            return null;
        const splitListener = this.splitListener;
        /**
    ...
    getSuggestionAtCaretPosition(input, caretPosition) {
        ...
        if (!minimumParser)
            return null;
        const { parser: sqlParserIns, tokenIndexOffset, newTokenIndex, parseTree: c3Context, } = minimumParser;
        const core = new CodeCompletionCore(sqlParserIns);
        core.preferredRules = this.preferredRules;
        const candidates = core.collectCandidates(newTokenIndex, c3Context);
        const originalSuggestions = this.processCandidates(candidates, allTokens, newTokenIndex, tokenIndexOffset);
